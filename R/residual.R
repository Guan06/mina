...
#' @import dplyr gdata
#' @param x An mina object with @dis and @des_tab defined.
#' @param group The name(s) of column(s) defined as experimental setup group(s).
#'
#' @examples
#' x <- com_r2(x, group = "Soil")
#'
#' m <- new("mina", dis = distance_matrix, des_tab = design_data_frame)
#' m <- com_r2(m)
#'
...
setMethod("com_r2", signature("mina", "character"), function(x, group) {
    r2 <- get_r2(x@dis, x@des_tab, group = group)
    return(r2)
})
...
get_r2 <- function(x, des, group = c("Host_genotype", "Compartment", "Soil",
    ...
    dis <- dis %>% mutate(distance_sq = distance ** 2)
    SSt <- sum(dis$distance_sq) / nrow(x)

    dis_intra <- dis %>% filter(sample1_group == sample2_group)
    lst <- levels(drop.levels(dis_intra$sample1_group))

    SSw <- 0
    for (l in lst) {
        this_group <- dis_intra %>% filter(sample1_group == l)
